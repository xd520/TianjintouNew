...
-(void)getScrollViewUI {
    ...
    UIView *viewthird= [[UIView alloc] initWithFrame:CGRectMake(0, 95 + 185 + 55*2, ScreenWidth, 45)];
 ...
 //银行卡密码
    ...
    UILabel *tiplabBankPW = [[UILabel alloc] initWithFrame:CGRectMake(10, 15, 70, 15)];
    tiplabBankPW.font = [UIFont systemFontOfSize:15];
    tiplabBankPW.textColor = [ColorUtil colorWithHexString:@"999999"];
    tiplabBankPW.text = @"银行卡密码";
    [viewfourh addSubview:tiplabBankPW];
    
    
    bankAccountPW = [[UITextField alloc] initWithFrame:CGRectMake(80,5, ScreenWidth - 100, 35)];
    bankAccountPW.backgroundColor = [UIColor whiteColor];
    
    bankAccountPW.placeholder = @"请输入交易密码";
    bankAccountPW.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    bankAccountPW.clearButtonMode = UITextFieldViewModeWhileEditing;
    bankAccountPW.textColor = [ColorUtil colorWithHexString:@"333333"];
    bankAccountPW.font = [UIFont systemFontOfSize:15];
    bankAccountPW.secureTextEntry = YES;
    bankAccountPW.delegate = self;
    ...
    sureBtn.frame = CGRectMake(10,385 + 70 + 55, ScreenWidth - 20, 35);
...
-(NSString *)digitUppercase:(NSString *)money
{
    ...
    NSArray *MyScale=@[@"分", @"角", @"圆", @"拾", @"佰", @"仟", @"万", @"拾万", @"佰万", @"仟万", @"亿", @"拾亿", @"佰亿", @"仟亿", @"兆", @"拾兆", @"佰兆", @"仟兆" ];
    ...
    if (tag== kBusinessTagGetJRReCharger) {
        ...
        if ([[jsonDic objectForKey:@"success"] boolValue] == NO) {
            ...
            if ([jsonDic objectForKey:@"object"] == nil || [[jsonDic objectForKey:@"object"] count] == 0) {
                [self.navigationController.view makeToast:@"提现结果正在处理，请到转账充值中查询"];
            } else {
