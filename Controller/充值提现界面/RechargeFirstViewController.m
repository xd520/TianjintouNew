...
@interface RechargeFirstViewController ()

@end

@implementation RechargeFirstViewController
...
- (void)viewDidLoad
{
    ...
    _inPutMoney.keyboardType = UIKeyboardTypeDecimalPad;
    _inPutMoney.text = @"";
...
-(void)endPost:(NSString *)result business:(kBusinessTag)tag{
    ...
    if (tag == kBusinessTagGetJRisNeedPsw){
        ...
        if ([[jsonDic objectForKey:@"success"] boolValue]== YES) {
            ...
            vc.dic = self.dic;
            //vc.inputMoney = _inPutMoney.text;
...
- (IBAction)reChaargeMethods:(id)sender {
    [self.view endEditing:YES];
    ...
    //_inPutMoney.text = @"";
        ...
        if ([[[self.dic objectForKey:@"cgywcsResult"] objectForKey:@"FID_YHMMXY"] isEqualToString:@"1"]) {
            AddRechargeViewController *vc = [[AddRechargeViewController alloc] init];
            vc.hidesBottomBarWhenPushed = YES;
            vc.dic = self.dic;
