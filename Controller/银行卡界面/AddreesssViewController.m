...
@interface AddreesssViewController ()
{
...
@implementation AddreesssViewController
...
- (void)viewDidLoad
{
    ...
    dataList = [[NSMutableArray alloc] initWithObjects:@"男",@"女", nil];
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        [[NetworkModule sharedNetworkModule] postBusinessReqWithParamters:paraDic tag:kBusinessTagGetJRProvinceData owner:self];
...
- (void)slimeRefreshStartRefresh:(SRRefreshView *)refreshView
{
    ...
    [[NetworkModule sharedNetworkModule] postBusinessReqWithParamters:paraDic tag:kBusinessTagGetJRProvinceDataAgain owner:self];
}
    ...
    if ([indexPath row] == [dataList count]) {
        ...
        if (cell == nil) {
           ...
           // brandLabel.text = [[dataList objectAtIndex:[indexPath row]] objectForKey:@"XZQYMC"];
            brandLabel.text = [dataList objectAtIndex:indexPath.row];
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == [dataList count]) {
        ...
        /*
        if ([[[dataList objectAtIndex:indexPath.row] objectForKey:@"XZQYMC"] isEqualToString:@"中央单位"]) {
            self.logiVC.address.text = [[dataList objectAtIndex:indexPath.row] objectForKey:@"XZQYMC"];
            self.logiVC.addressDic = [dataList objectAtIndex:indexPath.row];
            [self.navigationController popViewControllerAnimated:YES];
            
        } else {
        CityViewController *cv = [[CityViewController alloc] init];
        cv.hidesBottomBarWhenPushed = YES;
        cv.loginVC = self.logiVC;
        self.logiVC.addressDic = [dataList objectAtIndex:indexPath.row];
        cv.strCode = [[dataList objectAtIndex:indexPath.row] objectForKey:@"XZQYDM"];
         cv.strTitle = [[dataList objectAtIndex:indexPath.row] objectForKey:@"XZQYMC"];
        ...
        self.logiVC.falemLab.text = [dataList objectAtIndex:indexPath.row];
        [self.navigationController popViewControllerAnimated:YES];
...
- (void)reloadCityData:(NSDictionary *)_code{
...
-(void)endPost:(NSString *)result business:(kBusinessTag)tag{
    ...
    } else if (tag == kBusinessTagGetJRProvinceDataAgain){
