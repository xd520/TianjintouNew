...
-(void)reloadDataWith:(NSDictionary *)dataArr {
   ...
   UILabel *totalM = [[UILabel alloc] init];
    totalM.text = [NSString stringWithFormat:@"支付总额%@元",[self AddComma:_str]];
...
- (void)requestLogin:(NSString *)_jymm tag:(kBusinessTag)_tag
{
    ...
    [paraDic setObject:_str forKey:@"wtje"];//委托金额除 以委托价格
...
-(void)initButten {
    for (int i = 0; i < [dataList count]; i++) {
        ...
        dateLab.text = [NSString stringWithFormat:@"%@日到期",[[dataList objectAtIndex:i] objectForKey:@"JSRQ"]];
   ...
   // [self refreshPrice];
    ...
    totalMoney.text = [NSString stringWithFormat:@"支付总额%@元",[self AddComma:_str]];
 ...
 //交易密码
    sureText1 = [[UITextField alloc] initWithFrame:CGRectMake(10, 25*dataList.count +60 + 15, ScreenWidth - 110, 35)];
    sureText1.backgroundColor = [UIColor whiteColor];
    sureText1.placeholder = @"交易密码";
    sureText1.layer.borderColor = [ColorUtil colorWithHexString:@"eeeeee"].CGColor;
    sureText1.layer.cornerRadius = 4;
    sureText1.layer.borderWidth = 1;
    sureText1.layer.masksToBounds = YES;
    ...
    sureText1.clearButtonMode = UITextFieldViewModeWhileEditing;
     sureText1.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    sureText1.font = [UIFont systemFontOfSize:15];
   
     sureText1.delegate = self;
    
    [lastView addSubview:sureText1];
...
- (void)refreshPrice
{
    ...
    [totalMoney setText:[NSString stringWithFormat:@"支付总额%d元",(int)[_str floatValue] - price]];
    }
    CGSize titleSize = [totalMoney.text sizeWithFont:totalMoney.font constrainedToSize:CGSizeMake(MAXFLOAT, 15)];
    totalMoney.frame = CGRectMake(10, 25*dataList.count + 30 + 15,titleSize.width, 15);
...
- (IBAction)sureMehtods:(id)sender {
    ...
    if ([sureText.text isEqualToString:@""]) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            ...
            [self requestLogin:sureText.text tag:kBusinessTagGetJRDoTrade];
...
- (IBAction)sureMehtods1:(id)sender {
    ...
    if ([sureText1.text isEqualToString:@""]) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            ...
            [self requestLoginMore:sureText1.text withWTSl:[self getId:dataList] tag:kBusinessTagGetJRDoTrade];
